...
def run_embedding(
    ...
    linear = Embedding(vocab_size, d_model, device=weights.device, dtype=weights.dtype)
    linear.load_state_dict({"embeddings": weights})
...
def run_swiglu(
    ...
    from cs336_basics import SwiGLUFFN
    device, dtype = w1_weight.device, w1_weight.dtype
    swiglu = SwiGLUFFN(d_model, d_ff, device, dtype)
...
def run_transformer_block(
    ...
    block.load_state_dict({
        "attention.Wqkv.W": torch.cat([weights["attn.q_proj.weight"], weights["attn.k_proj.weight"], weights["attn.v_proj.weight"]], dim=0),
        "attention.Wo.W": weights["attn.output_proj.weight"],
...
def get_adamw_cls() -> Any:
    ...
    """
    from cs336_basics import AdamWOptim
    return AdamWOptim
